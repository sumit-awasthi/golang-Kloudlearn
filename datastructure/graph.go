...
func (g Graph) CreatePath(firstNode, secondNode string) bool {
	...
	for len(q) > 0 {
		...
		if contains(edges, secondNode) {
			...
			fmt.Println("BFS implementation")
			fmt.Println(strings.Join(path, "->"))
			fmt.Println("Current Node is :-",currentNode)
			fmt.Println("Path Node is :-",path)
			return true
		}
...
func main() {
	...
	g.AddEdge("Shyam", "Sumit")
	g.AddEdge("Sumit", "Shyam")
	g.AddEdge("Tarun", "Sumit")
	g.AddEdge("Tarun", "Ram")
	g.AddEdge("Ram", "Tarun")
	g.AddEdge("Ram", "Shyam")

	g.DFS("Tarun")
	g.CreatePath("Tarun", "Shyam")
	fmt.Println("Elements in the graph:-",g)
